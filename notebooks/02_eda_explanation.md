...
**The `emoji` package is imported with a `!pip install` fallback.** That line is
for Kaggle. Locally it is already in `requirements.txt`.
...
ever diverge further, the regex is the one to distrust.

**Nothing here modifies the corpus.** The notebook adds helper columns to the
