...
**This is the only stage the UI strictly needs.** `04` produces the write-up
tables; the app reads `models/` and `results/thresholds.json` directly, with no
copying step.

**Keras version drift is real.** The shipped models were saved by Kaggle's Keras
3.13.2. Loading them on Keras 3.11 fails with:
...
therefore pins `keras>=3.13,<3.14`.
...
**Both tracks are trained, both are saved, neither is discarded.** The app lets
the user pick a track at inference time, which is only possible because all four
checkpoints exist.
